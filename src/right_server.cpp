...
bool add(sensor_msgs::SetCameraInfo::Request  &req,
   ...
   string filename = PATH + to_string(t) + "_right.yaml";
   ...
   out << YAML::Key << "camera_name" << YAML::Value << "narrow_stereo/right";
   ...
   //2nd file
   string rightfile = PATH + "right_latest.yaml";
   ofstream file(rightfile);
   file << out.c_str() << endl;

   res.success = true;
   res.status_message = "Stat_here";
...
{
  ros::init(argc, argv, "set_right_camera_info_server");
  ros::NodeHandle n;

  ros::ServiceServer service = n.advertiseService("right_camera/set_camera_info", add);
  ROS_INFO("Right Server is Up");
  ros::spin();
